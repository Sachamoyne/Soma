 ...
 */
router.post("/parse-vocabulary", async (req, res) => {
    try {
 ...
 */
function buildVocabularyParsingPrompt(sourceLanguage, targetLanguage) {
 ...
 */
function fallbackParseVocabulary(text) {
    const entries = [];
        ...
        for (const sep of separators) {
            if (line.includes(sep)) {
                ...
                if (parts.length >= 2 && parts[0].length > 0 && parts[1].length > 0) {
                    ...
                    if (parts[0].length > 50 || parts[1].length > 100)
                        continue;
                    if (/^\d+\.?\s*$/.test(parts[0]))
