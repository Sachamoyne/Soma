...
exports.generateCardsPreview = generateCardsPreview;
exports.confirmAndInsertCards = confirmAndInsertCards;
const zod_1 = require("zod");
const supabase_js_1 = require("@supabase/supabase-js");
const MAX_TEXT_LENGTH = 20000;
const MAX_PDF_ANALYSIS_LENGTH = 8000;
function truncatePdfAnalysisText(text) {
...
const CONCEPT_TYPE_PRIORITY = ["definition", "relation", "concept", "process", "fact"];
...
const analysisOutputSchema = zod_1.z.object({
    concepts: zod_1.z.array(zod_1.z.object({
        ...
        type: zod_1.z.enum(["definition", "concept", "relation", "process", "fact"]),
 ...
 */
async function generateCardsFromAnalysis(analysis, options) {
    const model = process.env.LLM_MODEL || "gpt-4o-mini";
    const baseURL = process.env.LLM_BASE_URL || "https://api.openai.com/v1";
...
- Back : réponse complète mais pas trop longue (2-3 phrases).`;
    ...
    const conceptsList = conceptsToUse.map(c => `- [${c.id}] ${c.name} (${c.type}): ${c.definition}`).join("\n");
 ...
 */
async function generateMissingConceptCards(missingConcepts, detailLevel) {
    ...
    const conceptsList = missingConcepts.map(c => `- [${c.id}] ${c.name} (${c.type}): ${c.definition}`).join("\n");
 ...
 */
async function generateWithPipeline(text, options) {
...
const strictOutputSchema = zod_1.z
    .object({
...
function hasDistinctConcepts(cards) {
...
async function callLLM(text, isRetry, options, retryContext) {
    const model = process.env.LLM_MODEL || "gpt-4o-mini";
    const baseURL = process.env.LLM_BASE_URL || "https://api.openai.com/v1";
    const cardsCount = options?.cardsCount;
    const detailLevel = options?.detailLevel || "standard";
...
Réponds UNIQUEMENT avec le JSON strict conforme au schéma, sans aucun texte supplémentaire.`;
    ...
    try {
        ...
        const dynamicOutputSchema = zod_1.z
            .object({
            language: zod_1.z.enum(["fr", "en"]),
            title: zod_1.z.string().min(1),
            cards: zod_1.z
        ...
        if (parsed && typeof parsed === "object" && Array.isArray(parsed.cards)) {
            ...
            if (validCards.length >= requiredMinCards) {
                ...
                const relaxedSchema = zod_1.z.object({
                    language: zod_1.z.enum(["fr", "en"]).default("fr"),
                    title: zod_1.z.string().min(1).default("Deck"),
                    cards: zod_1.z.array(zod_1.z.object({
                        front: zod_1.z.string().min(1),
                        back: zod_1.z.string().min(1),
                        tags: zod_1.z.array(zod_1.z.string()).optional(),
                        difficulty: zod_1.z.union([zod_1.z.literal(1), zod_1.z.literal(2), zod_1.z.literal(3), zod_1.z.literal(4), zod_1.z.literal(5)]).optional(),
    ...
    if (!hasDistinctConcepts(validated.cards)) {
        throw new Error("Cards do not test distinct concepts - too many duplicates detected");
    }
    return validated;
}
/**
 ...
 */
async function checkUserQuota(userId, cardCount = 10) {
    ...
    if (!userProfile) {
        ...
        const { data: newProfile, error: createError } = await adminSupabase
            ...
            ai_quota_reset_at: new Date(new Date().getFullYear(), new Date().getMonth() + 1, 1).toISOString(),
        })
            .select()
            .single();
        if (createError) {
    ...
    const planMonthlyLimits = {
        starter: 300,
        pro: 1000,
    };
    const targetLimit = plan === "starter" || plan === "pro" ? planMonthlyLimits[plan] : 0;
    const shouldUpdateLimit = (plan === "starter" || plan === "pro") && limit < targetLimit;
 ...
 */
async function generateCardsPreview(text, deckId, userId, options) {
 ...
 */
async function confirmAndInsertCards(input) {
    ...
    if (!quotaCheck.isFounderOrAdmin && quotaCheck.profile) {
        ...
        await adminSupabase
            .from("profiles")
            .update({
            ai_cards_used_current_month: (quotaCheck.profile.ai_cards_used_current_month || 0) + actualCardCount,
        })
            .eq("id", userId);
    }
    return {
...
//# sourceMappingURL=ai-cards.js.map